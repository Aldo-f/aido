...
describe('fetchModels', () => {
  it('returns cached results without calling fetch again', async () => {
     ...
     const first = await fetchModels('opencode', 'sk-testkey12345678', false);
     expect(first).toHaveLength(2);
     expect(first[0].id).toBe('big-pickle');
     expect(fetchSpy).toHaveBeenCalledTimes(1);

     // Second call should use cache, no extra fetch
     const second = await fetchModels('opencode', 'sk-testkey12345678', false);
   ...
   it('force sync bypasses cache', async () => {
    ...
    await fetchModels('opencode', 'sk-testkey12345678', false);
    await fetchModels('opencode', 'sk-testkey12345678', true); // force
   ...
   it('throws on network error', async () => {
     ...
     await expect(fetchModels('opencode', 'sk-testkey12345678', false)).rejects.toThrow('Could not reach opencode');
     vi.restoreAllMocks();
   });
   ...
   it('throws on non-ok response', async () => {
     ...
     await expect(fetchModels('opencode', 'sk-testkey12345678', false)).rejects.toThrow('401');
     vi.restoreAllMocks();
   });
